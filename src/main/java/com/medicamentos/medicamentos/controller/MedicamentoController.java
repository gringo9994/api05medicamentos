...
// Request metodo GET ,buscar informações
	...
	public void salvar( @RequestBody Medicamento medicamento) {
		...
		List<Long> idsReacoes = medicamento.getReacoes().stream().map(Reacoes::getId).collect(Collectors.toList());

		List<Reacoes> reacoes = reacoesRepository.findAllById(idsReacoes);

		medicamento.setReacoes(reacoes);

		medicamentoRepository.save(medicamento);
		// persiste automaticamente a entidade MEDICAMENTO
	...
	public void atualizar( @RequestBody Medicamento medicamento, @PathVariable("id") Long id) {
